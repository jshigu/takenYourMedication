...
struct TagviewRecord: View {
    ...
    //日付が変わる："O"　　変わらないと：" "
//    @ObservedObject var dateModelFlg = ChildViewModelClass()
    @State var dateModelFlg = " "
    ...
    var body: some View {
        VStack {
            ...
            HStack{
                Text("\(String(format: "%02d", month))月\(String(format: "%02d", day))日\(dateModelFlg)").font(.system(size: 18))
                    .font(.system(size: 20))
                    .frame(width:85, alignment: .center)
                    .offset(x: 0, y: 7 )
                    .onAppear {
                        Timer.scheduledTimer(withTimeInterval: 1200, repeats: true) { _ in
                            if dayUpdeteFlg == true {
                                dateModelFlg = "O"
                                dayUpdeteFlg = false
                            }else {
                                dateModelFlg = "*"
